# Aribot — Full Architecture & Product Design Context

## What this is

I own a crypto futures trading bot called Aribot (`usdt_paper_bot_v2.py`). It trades Bybit USDT perpetual swaps using a WMA-based strategy. It has three modes: `paper` (simulated), `shadow` (real orders dry-run), `live` (real money).

We are designing a no-code GUI that lets retail traders visually assemble a trading bot strategy, which the tool then packages as a downloadable ZIP (strategy config JSON + fixed engine code) they can deploy themselves.
...
**8. State restoration**
- Reloads all open positions from SQLite into memory (including partial exits, trailing stop state, native stop flags)
...
A no-code GUI where retail traders visually assemble a trading bot strategy. The tool generates a downloadable ZIP containing a strategy config file (JSON/YAML) + the fixed Aribot engine. The user deploys and runs it themselves.

**The visual tool only produces the config. The engine stays unchanged and reads it at runtime.**

### Output package (ZIP download)
```
mybot/
  strategy.json        ← what the UI generates
  bot/                 ← fixed engine files
  .env.example
  requirements.txt
  Dockerfile
  README.md            ← "how to run in 3 steps"
```

### UI structure: structured pipeline, not blank canvas
Retail traders need a pre-structured flow with fixed stages and optional slots, not a free-form canvas.

```
[MARKET] → [SIGNAL] → [FILTERS] → [RISK] → [EXITS]
```

Each stage has required slots (must be filled) and optional slots (add/remove blocks). Config panel appears on the right when a block is selected.

### Block taxonomy in retail-trader language

**MARKET stage**
- Exchange Connector *(Bybit, Binance Futures...)*
- Symbol Picker *(All USDT perps / pick specific coins / upload list)*
- Volume Filter *(Only trade coins with 24h volume above $X)*

**SIGNAL stage**
- Indicator *(Moving Average — type: WMA/EMA/SMA, period, source: close/OHLC4, timeframe)*
- Signal Rule *(Price above indicator = Long, below = Short)*
- Signal Confirmation *(Require higher high/lower low before entry)*
- Market Trend Gate *(Only trade in direction BTC is trending)*

**FILTERS stage** *(all optional)*
- Timeframe Gate *(Only enter at 4H candle close)*
- Freshness Guard *(Skip coins with stale or frozen price data)*

**RISK stage**
- Position Sizer *(% of balance per trade, leverage by coin tier)*
- Max Open Positions *(cap at N simultaneous trades)*
- Daily Loss Limit *(stop new entries if down X% on the day)*
- Consecutive Loss Cooldown *(pause after N losses in a row)*

**EXITS stage**
- Hard Stop Loss *(close if down X%)*
- Trailing Stop *(activate at X% profit, trail by Y%)*
- Scale Out *(take partial profits at level 1, 2, 3)*
- Time Exit *(close after N hours regardless)*
- Signal Flip Exit *(close when indicator signal reverses)*

**NOTIFICATIONS** *(sidebar)*
- Telegram Alerts *(bot token + chat ID)*
- Heartbeat Pulse *(send status every N hours)*

### Key UX rules
- **Paper mode is the default.** User must explicitly toggle to live before generating. Show a warning when they do.
- **Plain language everywhere.** No technical parameter names. Every numeric input has a plain-language label, tooltip, and sensible default.
- **One-click deploy target.** README includes a Render/Railway deploy button so users with no terminal experience can get running.
- **Start from a template.** The default assembled flow is the Aribot strategy as-is. Users tweak it, they don't build from scratch.

---

*Continue from here: design the screen layouts, config schema, and block connection rules. Ask me what I want to do first*
